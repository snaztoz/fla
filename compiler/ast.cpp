...
namespace fla::compiler::ast
{
    ...
    const std::string Arena::get_node_repr(const NodeIndex ni) const
    {
        return std::visit(
            util::overloaded {
                ...
                [this](const TypeNotation &n) -> std::string {
                    return std::format("type notation `{}`", get_type_notation_repr(n.tn));
                },
    ...
    const std::string Arena::get_type_notation_repr(const NodeIndex ni) const
    {
        return std::visit(
            util::overloaded {
    ...
    const Metadata &Arena::get_node_metadata(const NodeIndex ni) const
    {
